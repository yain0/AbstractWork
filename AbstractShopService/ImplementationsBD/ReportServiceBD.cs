...
winword.Documents.Add(ref missing, ref missing, ref missing, ref missing);
                ...
                var products = context.Remonts.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].RemontName;
...
excelcells.get_Offset(elem.Materials.Count() - 1, 1));
                        ...
                        excelcells.Value2 = elem.TotalKoll;
                        excelcells.Font.Bold = true;
                    }
                }
                //сохраняем
        ...
        public void SaveCustomerActivitys(ReportBindingModel model)
        {
            ...
            {
                File.WriteAllBytes("TIMCYR.TTF", AbstractShopService.Properties.Resources.TIMCYR);
            }
...
new iTextSharp.text.Font(baseFont, 14, iTextSharp.text.Font.BOLD));
            ...
            //заполняем таблицу
            var list = GetCustomerActivitys(model);
            ...
            for (int i = 0; i<list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].RemontName, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].Koll.ToString(), fontForCells));
