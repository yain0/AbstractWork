...
namespace AbstractWorkView
{
    public partial class FormSklads : Form
    {
        ...
        private void LoadData()
        {
            try
            {
                List<SkladViewModel> list = Task.Run(() => APICustomer.GetRequestData<List<SkladViewModel>>("api/Sklad/GetList")).Result;
                if (list != null)
                {
        ...
        private void buttonDel_Click_1(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                if (MessageBox.Show("Удалить запись", "Вопрос", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    Task task = Task.Run(() => APICustomer.PostRequestData("api/Sklad/DelElement", new CustomerBindingModel { Id = id }));
