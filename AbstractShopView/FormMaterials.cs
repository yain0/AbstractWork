...
namespace AbstractWorkView
{
    public partial class FormMaterials : Form
    {
        ...
        private void LoadData()
        {
            try
            {
                List<MaterialViewModel> list = Task.Run(() => APICustomer.GetRequestData<List<MaterialViewModel>>("api/Material/GetList")).Result;
                if (list != null)
                {
        ...
        private void buttonDel_Click_1(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                if (MessageBox.Show("Удалить запись", "Вопрос", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    Task task = Task.Run(() => APICustomer.PostRequestData("api/Material/DelElement", new CustomerBindingModel { Id = id }));
